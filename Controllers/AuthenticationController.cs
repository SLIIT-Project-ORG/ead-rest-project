		...
		private async Task <RegisterResponse> RegisterAsync(RegisterRequest request)
		{
			try
			{
				...
				if (userExists != null)
					...
					return new RegisterResponse { Message = "Invalid email/ password", Success = false };
					...
					//user is created
					// then add user to arole
					var addUserToRoleResult = await _userManager.AddToRoleAsync(userExists, "USER");

					if(!createUserResult.Succeeded) return new RegisterResponse {Message =$"Create User Succeeded but could not add user to role{addUserToRoleResult? .Errors ? .First()? .Description}", Success = false};
